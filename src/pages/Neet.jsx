...
function Neet() {
  ...
  const onUpload = () => {
    //registered
    const uidd = uid();
    const path = ref(db, "neet/" + year + "/" +sub + "/" + uidd);
    console.log(path);
    set(path, {
      question: question,
      op1: op1,
  ...
  useEffect(() => {
    const fetchData = async () => {
      const tempref = "neet/" + year;
      const dataRef = ref(db, "neet/" +year+ "/"+sub); // Replace with your reference
  ...
  return (
    <div className="mx-8">
              <h1 className="w-full text-center text-xl sm:text-lg" >NEET</h1>
      ...
      <div>
        {data &&
          Object.values(data).map((item) => (
            <Question item={item} year={year} sub={sub} exam="neet" /> // Adjust based on your data structure
          ))}
      </div>
    </div>
  );
