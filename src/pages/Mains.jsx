...
function Mains() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const tempref = "mains/" + year;
      const dataRef = ref(db, "mains/" +year+ "/"+sub); // Replace with your reference
      ...
      <div>
        {data &&
          Object.values(data).map((item) => (
            <Question item={item} year={year} sub={sub} exam="mains" /> // Adjust based on your data structure
          ))}
      </div>
    </div>
  );
